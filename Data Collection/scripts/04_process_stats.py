...
def process_stats(target_slug=None, target_division=None):
    # Script is now in scripts/ folder
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    data_dir = os.path.join(base_dir, 'data')
    
    # Division folders
    division_folders = ['Division I', 'Division II', 'Division III']
    if target_division:
        div_map = {'DI': 'Division I', 'DII': 'Division II', 'DIII': 'Division III'}
        division_folders = [div_map.get(target_division, target_division)]
    
    # 1. Load Team Stats
    team_stats_map = {}
    
    print("Loading team stats...")
    for div_folder in division_folders:
        div_path = os.path.join(data_dir, div_folder)
        if not os.path.exists(div_path):
            continue
            
        schools = [d for d in os.listdir(div_path) if os.path.isdir(os.path.join(div_path, d))]
        
        for slug in schools:
            team_dir = os.path.join(div_path, slug, 'team')
    ...
    for div_folder in division_folders:
        ...
        for slug in schools:
            players_dir = os.path.join(div_path, slug, 'players')
            roster_path = os.path.join(div_path, slug, 'roster.json')
    ...
    cols = [
        'MasterRank', 'TeamRank', 'Division', 'TeamSlug', 'PlayerName', 'Number', 'ClassYear', 'RosterPosRaw', 'PosGroup',
    ...
    # Write Team CSVs (grouped by division and team)
    teams_data = {}  # (division, slug) -> players
    for p in all_players:
        key = (p['Division'], p['TeamSlug'])
        if key not in teams_data: teams_data[key] = []
        teams_data[key].append(p)
        
    for (div, slug), players in teams_data.items():
        for i, p in enumerate(players):
            p['TeamRank'] = i + 1
            
        csv_path = os.path.join(data_dir, div, slug, f"{slug}_ranked.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--division', type=str, choices=['DI', 'DII', 'DIII'],
                        help='Only process teams from this division')
    args = parser.parse_args()
    
    process_stats(args.target, args.division)
